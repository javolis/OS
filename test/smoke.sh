...
# Avolis shell: lock -> unlock -> desktop (cycle the taskbar edge with 'p')
# -> command palette (open with '/', type "date", Enter launches date.elf
# via sys_spawn_fg). CI drives it (Enter, p, /, d,a,t,e, Enter, q) and checks
# the transitions plus the real launch over serial.
if grep -q "avolis: unlocked" "$SERIAL_LOG" \
    ...
    echo "PASS: Avolis desktop, taskbar, and command palette launch"
else
    echo "FAIL: Avolis desktop/palette interactions broke" >&2
    fail=1
fi
